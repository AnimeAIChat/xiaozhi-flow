 ...
 */
export const convertReactFlowNodeToRete = (reactNode: ReactFlowNode): ReteNode => {
...
export const convertReactFlowEdgeToRete = (
  ...
  try {
    ...
    if (!sourceNode || !targetNode) {
      console.warn(`无法找到连接的节点: ${reactEdge.source} -> ${reactEdge.target}`);
      return null;
    }
...
export const convertReactFlowToRete = (
  ...
  try {
    // 转换节点
    const reteNodes: ReteNode[] = nodes.map(node => convertReactFlowNodeToRete(node));
 ...
 */
export const convertReteNodeToReactFlow = (reteNode: ReteNode): ReactFlowNode => {
  return {
    id: reteNode.id,
...
export const convertReteToReactFlow = (
  ...
  try {
    const reactFlowNodes: ReactFlowNode[] = nodes.map(node => convertReteNodeToReactFlow(node));
    const reactFlowEdges: ReactFlowEdge[] = connections.map(conn => convertReteConnectionToReactFlow(conn));
