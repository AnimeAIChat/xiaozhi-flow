...
import { usePluginList, usePluginStatuses, usePluginServices } from '../../stores/useAppStore';
...
export const PluginList: React.FC<PluginListProps> = ({
  ...
  // 获取插件状态
  const getPluginStatus = useCallback((pluginId: string): PluginStatus | undefined => {
    return pluginStatuses[pluginId];
  }, [pluginStatuses]);

  // 获取插件服务
  const getPluginServices = useCallback((pluginId: string): ServiceInfo[] => {
    return Object.values(pluginServices).filter(service => service.pluginId === pluginId);
  }, [pluginServices]);
  ...
  const filteredPlugins = useMemo(() => {
    return Object.values(installedPlugins).filter(plugin => {
      ...
      if (filterStatus !== 'all') {
        ...
        if (filterStatus === 'active' && (!status || status.status !== 'active')) return false;
        if (filterStatus === 'inactive' && (!status || status.status !== 'inactive')) return false;
        if (filterStatus === 'error' && (!status || status.status !== 'error')) return false;
      }
      ...
      // 分类过滤
      if (filterCategory !== 'all' && plugin.metadata.category !== filterCategory) {
        return false;
      }

      return true;
    });
  }, [installedPlugins, searchText, filterStatus, filterType, filterCategory, getPluginStatus]);
  ...
  const getPluginIcon = useCallback((plugin: IPlugin) => {
    ...
    return iconMap[plugin.metadata.category] || plugin.metadata.icon || <AppstoreOutlined />;
  }, []);

  // 获取状态标签
  const getStatusTag = useCallback((plugin: IPlugin) => {
    const status = getPluginStatus(plugin.id);
    if (!status) {
    ...
    switch (status.status) {
      ...
      case 'loading':
      case 'activating':
        return <Tag color="processing" icon={<ReloadOutlined />}>启动中</Tag>;
      case 'deactivating':
        return <Tag color="processing" icon={<ReloadOutlined />}>停止中</Tag>;
      case 'error':
  ...
  // 获取服务状态
  const getServiceStatus = useCallback((plugin: IPlugin) => {
    ...
    return (
      <Space size="small">
        ...
        {runningServices.length === totalServices && runningServices.length > 0 && (
          <Tag color="green" icon={<CheckCircleOutlined />}>全部运行</Tag>
        )}
        {runningServices.length === 0 && totalServices > 0 && (
          <Tag color="red" icon={<CloseCircleOutlined />}>全部停止</Tag>
        )}
        {runningServices.length > 0 && runningServices.length < totalServices && (
          <Tag color="orange" icon={<ExclamationCircleOutlined />}>部分运行</Tag>
        )}
      </Space>
    );
  }, [getPluginServices]);
  ...
  const columns: ColumnsType<IPlugin> = [
    ...
    },
    {
      ...
      render: (type: string, record: IPlugin) => (
        <Tag color={type === 'backend' ? 'red' : 'blue'}>
          {type}
        </Tag>
      )
    },
    {
      ...
      render: (runtime: string) => (
        <Tag color={runtime === 'python' ? 'green' : runtime === 'go' ? 'orange' : 'blue'}>
          {runtime}
        </Tag>
      )
    },
      ...
      render: (_, record: IPlugin) => {
        ...
        return (
          <Switch
            ...
            loading={status?.status === 'loading' || status?.status === 'activating'}
          />
        );
      }
    },
