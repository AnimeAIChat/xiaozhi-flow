  ...
  implements RuntimeAdapter
{
  ...
  async start(config: DockerServiceConfig): Promise<ServiceInfo> {
    try {
      ...
      // 等待服务启动
      const serviceInfo = await this.waitForContainerReady(
        containerInfo,
        config,
      );
  ...
  async stop(serviceInfo: ServiceInfo): Promise<void> {
    try {
      ...
      const containerInfo = Array.from(this.containers.values()).find(
        (container) =>
          container.labels &&
          container.labels['plugin-id'] === serviceInfo.pluginId,
      );
  ...
  private async validateDockerEnvironment(): Promise<DockerEnvironment> {
    ...
    try {
      ...
      // 获取Docker API版本
      const infoResult = await this.executeDockerCommand([
        'version',
        '--format',
        '{{.Server.APIVersion}}',
      ]);
      ...
      throw new Error(
        'Docker environment not available. Please ensure Docker is installed and running.',
      );
    }
  }
  ...
  private async startContainer(
    ...
    const containerName =
      config.containerName || `plugin-${config.pluginId}-${Date.now()}`;
    ...
    if (config.volumes) {
      Object.entries(config.volumes).forEach(([hostPath, mountConfig]) => {
        runArgs.push(
          '-v',
          `${hostPath}:${mountConfig.bind}:${mountConfig.mode}`,
        );
      });
    }
  ...
  private async waitForContainerReady(
    ...
    while (Date.now() < maxTime) {
      ...
      if (updatedContainer && !updatedContainer.status.startsWith('Up')) {
        throw new Error(
          `Container stopped unexpectedly: ${updatedContainer.status}`,
        );
      }
