...
// TTSProvider 豆包 TTS 提供者
type TTSProvider struct {
	*BaseTTS
	baseURL string
}

// NewTTSProvider 创建豆包 TTS 提供者
func NewTTSProvider(config *TTSConfig, deleteFile bool) (*TTSProvider, error) {
	base := NewBaseTTS(config, deleteFile)
...
// ToTTS 实现文本到语音的转换
func (p *TTSProvider) ToTTS(text string) (string, error) {
	// 创建WebSocket连接
...
func (p *TTSProvider) parseResponse(res []byte) (resp synResp, err error) {
	if len(res) < 4 {
