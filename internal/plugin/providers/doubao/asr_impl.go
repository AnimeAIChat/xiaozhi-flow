...
func (p *ASRProvider) Transcribe(ctx context.Context, audioData []byte) (string, error) {
	if p.isStreaming {
...
func (p *ASRProvider) generateHeader(
	messageType uint8,
	flags uint8,
...
func (p *ASRProvider) constructRequest() map[string]interface{} {
...
func (p *ASRProvider) GetAudioBuffer() *bytes.Buffer {
	return p.BaseASR.GetAudioBuffer()
}
...
func (p *ASRProvider) parseResponse(data []byte) (map[string]interface{}, error) {
	if len(data) < 4 {
...
// AddAudio 添加音频数据到缓冲区
func (p *ASRProvider) AddAudio(data []byte) error {
	return p.AddAudioWithContext(context.Background(), data)
}

// AddAudioWithContext 带上下文的音频数据添加
func (p *ASRProvider) AddAudioWithContext(ctx context.Context, data []byte) error {
	// 使用锁检查状态
	p.connMutex.Lock()
...
func (p *ASRProvider) StartStreaming(ctx context.Context) error {
...
func (p *ASRProvider) startPreConnect() {
...
func (p *ASRProvider) sendInitialRequest(ctx context.Context) {
	// 发送初始请求
...
func (p *ASRProvider) ReadMessage() {
...
func (p *ASRProvider) setErrorAndStop(err error) {
	p.connMutex.Lock()
	defer p.connMutex.Unlock()
	p.err = err
	p.isStreaming = false
...
func (p *ASRProvider) closeConnection() {
	defer func() {
		if r := recover(); r != nil {
			// 静默处理panic，避免程序崩溃
...
func (p *ASRProvider) SendLastAudio(data []byte) error {
	return p.sendAudioData(data, true)
}

// sendAudioData 直接发送音频数据，替代之前的sendCurrentBuffer
func (p *ASRProvider) sendAudioData(data []byte, isLast bool) error {
	p.logger.DebugTag(
		"ASR",
...
func (p *ASRProvider) Initialize() error {
	// 确保输出目录存在
...
func (p *ASRProvider) Cleanup() error {
	// 使用锁保护状态变更
	p.connMutex.Lock()
	defer p.connMutex.Unlock()
...
func (p *ASRProvider) CloseConnection() error {
	return nil
}
